// ==UserScript==
// @name         Gabung Data CSV & XLSX - epus
// @namespace    PKM
// @version      1.0
...
(function () {
  ...
  // === Filter URL: hanya aktif di halaman pelayanan EPUS Cirebon ===
  if (!/^https:\/\/cirebon\.epuskesmas\.id\/pelayanan\?searchKey=/.test(window.location.href)) {
    return;
  }

  // === Pastikan library XLSX sudah tersedia (harus di-@require oleh loader) ===
  if (typeof XLSX !== 'object') {
    console.warn('[epus-gabung-data] Library XLSX tidak tersedia. Pastikan loader menyediakannya.');
    return;
  }

  console.log("✅ Modul EPUS: Gabung Data aktif di:", window.location.href);
  ...
  async function collectAllPages(fromPage = 1, toPage = null) {
    try {
      ...
      // set perPage = 100 (default maksimal)
      ...
      if (perPageSelect) {
        ...
        await delay(1500); // tunggu reload
      }
      ...
      // pindah dulu ke halaman fromPage jika > 1
      if (fromPage > 1) {
        goToPage(fromPage);
        await delay(1500);
      ...
      while (currentPage <= toPage && !canceled) {
        console.log(`📥 Ambil data halaman ${currentPage}/${toPage}`);
        await delay(1200);
  ...
  function goToPage(pageNum) {
    const target = [...document.querySelectorAll("ul.pagination li.clickable span")]
      .find(el => el.innerText.trim() == pageNum);
